...
$number3 = 15439.093564654;
        ...
        foreach (json_decode($data['produtos']) as $produto) {
            ...
            $qtnEstimada = str_replace('.', "", $produto->dose_estimada_hectare);
            $qtnEstimada = str_replace(',', ".", $qtnEstimada);

            //$bombaRecomendada = str_replace('.', "", $model->bomba_recomendada);
            //$bombaRecomendada = str_replace(',', ".", $bombaRecomendada);

            $doseRecomendada = round(($qtnEstimada / $model->capacidade_bomba), 3);
            $qtnRecomenda = round($qtnEstimada * $model->bomba_recomendada, 3);

            $qtnReal = $qtnRecomenda;
            $doseReal = $doseRecomendada;

            if (isset($data['bomba_usada'])) {
                $qtnReal = round($model->bomba_usada * $qtnEstimada, 3);
                $doseReal = round(($qtnReal / $model->area), 3);
            }
            ...
            $dados = [
                ...
                'dosagem' => $doseRecomendada,
                'quantidade' => round($qtnRecomenda, 3),
                'dosagem_real' => $doseReal,
                'quantidade_real' => $qtnReal,
                'qtn_bomba' => $qtnEstimada,
            ];
            //dd($dados);
